...
class Task1:
    ...
    def scale_features(self, df):
        ...
        columns_to_scale = ['current_price', 'raw_price', 'discount', 'likes_count']
        numerical = df[columns_to_scale]
        scaled_features = self.scaler.fit_transform(numerical)
        scaled_features_df = pd.DataFrame(scaled_features, columns=columns_to_scale, index=self.df.index)
    ...
    def plot_correlation_matrix(self, df, alpha=0.05):
        ...
        for row in correlation_matrix.index:
            for col in correlation_matrix.index:
                if row == col:
                    p_values.loc[row, col] = 0.0  # p-value is 0 for diagonal elements
                else:
    ...
    def hypothesis3_likes_category(self , df):
        likes_count_by_category = [group['likes_count'].values for name, group in self.df.groupby('category')]
        ...
        df['category'] = self.le_category.inverse_transform(df['category'].to_numpy())
    ...
    def analyze_top_categories_and_products(self, df, top_n=7):
        ...
        def get_top_categories_by_likes(df, top_n):
            avg_likes_per_category = df.groupby('category')['likes_count'].mean().sort_values(ascending=False)
        ...
        print("Top 7 Categories (based on average likes count):", self.le_category.inverse_transform(top_categories))
        print()
        print("Top 10 Products in the Top 7 Categories:\n", self.le_name.inverse_transform(top_products['name'].to_list()))

        top_products['category'] = self.le_category.inverse_transform(top_products['category'].to_numpy())
        return top_products
...
class Task2:
    ...
    def perform_clustering(self):
        ...
        for name, algorithm in algorithms.items():
            ...
            self.metrics["Silhouette Score"][name] = silhouette_score(self.X, labels)
            self.metrics["Calinski-Harabasz Index"][name] = calinski_harabasz_score(self.X, labels)
            self.metrics["Davies-Bouldin Index"][name] = davies_bouldin_score(self.X, labels)
    ...
    def predict_cluster_kmeans(self, current_price, discount):
        ...
        if self.kmeans_model is None:
            raise ValueError("KMeans model has not been trained. Call perform_clustering() first.")
        input_data = pd.DataFrame({'current_price': [current_price], 'discount': [discount]})
    ...
    def predict_cluster_agglomerative(self, current_price, discount):
        ...
        if self.centroids is None:
            raise ValueError("Centroids have not been computed. Call perform_clustering() first.")
    ...
    def map_top_products_to_clusters(self, le_name, le_category):
        ...
        top_products_clusters = top_products_df[top_products_df['name'].isin(self.top_products['name'].to_list())]

        top_products_clusters['name'] = le_name.inverse_transform(top_products_clusters['name'].to_numpy())
        top_products_clusters['category'] = le_category.inverse_transform(top_products_clusters['category'].to_numpy())
    ...
    def visualize_clusters(self, df_copy, df_mean, labels, algorithm_name):
        ...
        top_products_df = df_copy[df_copy['name'].isin(self.top_products['name'].to_list())]
        ax.scatter(
    ...
    def print_metrics(self):
        ...
        best_algorithm = max(self.metrics["Silhouette Score"], key=self.metrics["Silhouette Score"].get)
...
class Task3:
    ...
    def preprocess_data(self, le_category):
        ...
        self.df['popularity'] = self.df['likes_count'].apply(lambda x: 1 if x > self.df['likes_count'].mean() else 0)
        
        # Selecting features and target
        print("Selecting features and target ['category', 'current_price', 'discount']")
    ...
    def train_knn(self):
        ...
        grid_search = GridSearchCV(knn, param_grid, cv=self.skf, scoring='accuracy')
        ...
        cv_scores = cross_val_score(self.knn_model, self.X_train, self.y_train, cv=self.skf, scoring='accuracy')
    ...
    def evaluate_knn(self):
        ...
        metrics = pd.DataFrame({'Metric': ['Precision', 'Recall', 'F1-Score'], 'Score': [precision, recall, f1]})
        plt.figure(figsize=(8, 6))
    ...
    def train_decision_tree(self):
        ...
        grid_search = GridSearchCV(dt, param_grid, cv=self.skf, n_jobs=-1, verbose=2)
        grid_search.fit(self.X_train, self.y_train)
        ...
        # Cross-validation scores
        cv_scores = cross_val_score(self.dt_model, self.X_train, self.y_train, cv=self.skf, scoring='accuracy')
...
def main():
    ...
    task2.print_metrics()
    top_products_clusters = task2.map_top_products_to_clusters(task1.le_name, task1.le_category)
    print(top_products_clusters[['name','category','kmeans_cluster','agglomerative_cluster']])
